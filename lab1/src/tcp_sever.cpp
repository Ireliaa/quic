/*
    测试:
    - 运行本程序。输入q回车退出
    - 启动客户端nc -u 127.0.0.1 5555, 或用本项目的tcpcli, 然后输入字符串回车
 */
#include <cstdio>
#include <cstring>
#include <cstdlib>
...
#define EP_WARN(EXPR, ...) EP_CHECK(EXPR, __VA_ARGS__)
...
int main(int argc, const char** argv)
{
    ...
    int listen_fd = 0;
    EP_ENSURE(listen_fd = socket(AF_INET, SOCK_STREAM, 0), EXIT_FAILURE);
    ...
    EP_ENSURE(listen(listen_fd, 5), EXIT_FAILURE);
    ...
    while (true) {
        ...
        for (int i = 0; i < nfds; ++i) {
            ...
            if (events[i].events & EPOLLIN && fd == listen_fd) {
                ...
                int cfd = -1;
                EP_WARN(cfd = accept(fd, (sockaddr*)&c_addr, &len));
                if (cfd >= 0) {
                    clients.insert(cfd);
                    char hbuf[NI_MAXHOST]{}, sbuf[NI_MAXSERV]{};
                    getnameinfo((sockaddr*)&c_addr, len, hbuf, sizeof(hbuf), sbuf, sizeof(sbuf), NI_NUMERICHOST | NI_NUMERICSERV);
                    printf("connect socket %d, from %s:%s\n", cfd, hbuf, sbuf);

                    // 把连接socket设置为非阻塞方式
                    int opts = 0;
                    EP_ENSURE(opts = fcntl(cfd, F_GETFL), EXIT_FAILURE);
                    EP_WARN(fcntl(cfd, F_SETFL, opts | O_NONBLOCK), EXIT_FAILURE);

                    ev.data.fd = cfd;
                    ev.events = EPOLLIN | EPOLLET;
                    EP_ENSURE(epoll_ctl(efd, EPOLL_CTL_ADD, cfd, &ev), EXIT_FAILURE); //
                }
                ...
                while (true) { // NOTE: 可以循环一直读到 EAGAIN, 未读完内容不会被丢弃. 也可以不用 EPOLLET, 下次wait会立即返回继续读
                    char buf[MAX_READ]{};
                    auto ret = recv(fd, &buf, sizeof(buf), 0);
                    if (ret == 0) {
                        // 对端断开连接
                        printf("disconnect fd %d\n", fd);
                        EP_WARN(epoll_ctl(efd, EPOLL_CTL_DEL, fd, nullptr));
                        close(fd);
                        break;
                    } else if (ret == -1) {
                        if (errno != EAGAIN)
